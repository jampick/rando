#!/usr/bin/env python3
"""
Fatality Data Processor
Processes OSHA fatality data and converts to our importable format
"""
...
class FatalityDataProcessor:
    """Process OSHA fatality data"""

    def __init__(self):
        self.raw_file = "../data/osha/raw/fatality_data.csv"  # Update this path
        self.processed_file = "../data/osha/processed/fatality_data_processed.csv"

        # Fatality to our schema mapping (adjust based on actual CSV columns)
        self.column_mapping = {
            'ID': 'osha_id',
    ...
    def process_fatality_data(self):
        ...
        logger.info("Starting fatality data processing...")
    ...
    def _map_record(self, row):
        ...
        try:
            ...
            # Map known columns
            for fatality_col, our_col in self.column_mapping.items():
                if fatality_col in row.index:
                    mapped[our_col] = row[fatality_col]

            # Set required fields with defaults for fatalities
            mapped['incident_type'] = 'fatality'  # Always fatality for this data
            mapped['investigation_status'] = 'Closed'  # Fatalities are typically closed
            ...
            # Process incident type (categorize by event type)
            if pd.notna(mapped.get('source')):
                mapped['incident_type'] = self._categorize_fatality(mapped['source'])
    ...
    def _categorize_fatality(self, source):
        """Categorize fatality type from source"""
        if pd.isna(source):
            return 'fatality'
    ...
    def _generate_summary(self, df):
        ...
        print("="*60)
        print(f"Total fatalities processed: {len(df):,}")
...
def main():
    ...
    processor = FatalityDataProcessor()

    print("💀 Processing OSHA Fatality Data")
    print("="*50)
